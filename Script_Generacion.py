...
machines = pd.DataFrame(
    {
        ...
        "machine_type": [
            random.choice(["CNC", "Ensamblaje", "Empaque", "Corte"])
            for _ in range(50)
        ],
        "install_date": [
            base_date - timedelta(days=random.randint(0, 3000)) for _ in range(50)
        ],
    }
)
    ...
    downtime.append(
        {
            ...
            "reason_code": random.choice(
                ["MEC-01", "MEC-02", "ELEC-01", "CAL-01", "MAT-01"]
            ),
            "order_id": (
                random.choice(orders_df["order_id"])
